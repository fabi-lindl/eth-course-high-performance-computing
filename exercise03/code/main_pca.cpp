...
int main (int argc, char** argv)
{
  ...
  utils::transposeData(data_T, data, N, D); // TODO (DONE): Implement the function found in utils
  ...
  utils::computeMean(data_mean, data_T, N, D);   // TODO (DONE): Compute mean in utils
  utils::computeStd(data_std, data_mean, data_T, N, D); // TODO (DONE): Compute standard deviation in utils
  ...
  if(scaler.compare("standard") == 0){
    utils::standardizeColMajor(data_T, data_mean, data_std, N, D);   // TODO (DONE): write scaler
  }
  ...
  printf("before dsyev\n"); 
  dsyev(&jobz, &uplo, &D, data_cov, &D, W, work, &lwork, &info);
  printf("after dsyev\n");
  printf("work = %f\n", work[0]);
  printf("work = %f\n", work[1]);
  ...
  utils::getEigenvectors(V, data_cov, num_comp, D); // TODO(DONE): Extract the eigenvectors from the last rows of data_cov
  delete[] data_cov;
  ...
  utils::reduceDimensionality(data_reduced, V, data_T, N, D, num_comp);   // TODO(DONE): 
  ...
  int dim_old = N*D;
  int dim_comp = N*num_comp + D*num_comp + D*2;
