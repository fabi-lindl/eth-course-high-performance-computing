...
struct Diffusion {
    ...
    void write_mpi_sequential(const std::string& filename) const
    {
        ...
        MPI_Gather((void *)&c[1 * fullN], nlocal, MPI_DOUBLE, &outputData[0], nlocal,
                   MPI_DOUBLE, 0, MPI_COMM_WORLD);

        // Saving data to file
        if (rank == 0) {
            auto file = fopen(filename.c_str(), "wb"); // Write-only, binary mode
            fwrite(outputData.data(), sizeof(outputData[0]), outputData.size(), file);
            fclose(file);
        }
    }

    void write_mpi_parallel(const std::string& filename) const
    //void write_mpi_parallel(char* filename) const
    {
        ...
        // Define variables. 
        len = local_N * (N+2);
        offset = len * rank * sizeof(double);
        start = N+2;
        
        // Create file with the concentration data. 
        // Open file (create file, writonly mode). 
        MPI_File_open(MPI_COMM_WORLD, (char *)filename.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file);
        MPI_File_set_size(file, N*(N+2)*sizeof(double));
        // Wite data to file. 
        MPI_File_write_at_all(file, offset, (void *)&c[start], len, MPI_DOUBLE, MPI_STATUS_IGNORE);
        // Close file. 
        MPI_File_close(&file);
    }
};
